...
local function is_solid(col_info) return col_info.type == "slide" end

local function collision_filter(col_info, id, ecs_world)
    local is_sold = is_solid(col_info)
...
function jump_control.create(ctx, player_entity_topic)
        ...
        collision = ctx:listen("collision")
            :filter(function(all_col_info)
                return collision_filter(
                    all_col_info, player_entity_topic:peek().id,
                    all_col_info.ecs_world
                )
            end)
            :foreach(function()
